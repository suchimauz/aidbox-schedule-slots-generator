...
func (l *CacheHitListener) startHealthcareServiceQueue(ctx context.Context) error {
	...
	l.logger.Info("rabbitmq.healthcare_service.setup_starting", nil)
	...
	go func() {
		...
		for {
			...
			case msg, ok := <-msgs:
				...
				err := l.processHealthcareServiceMessage(ctx, msg)
...
func (l *CacheHitListener) processHealthcareServiceMessage(ctx context.Context, msg amqp.Delivery) error {
	...
	if cacheMessageRoutingKey.ResourceType != CacheHitResourceTypeHealthcareService {
		l.logger.Debug("rabbitmq.message.skipped", out.LogFields{
			"expected": string(CacheHitResourceTypeHealthcareService),
	...
	l.logger.Info("healthcare_service.message.received", out.LogFields{
		"id":           msgJson.ID,
	...
	if cacheMessageRoutingKey.CacheHitType == CacheHitTypeInvalidate {
		...
		if err := l.useCase.InvalidateSlotsCache(invalidateCtx, msgJson.ID); err != nil {
			l.logger.Error("healthcare_service.invalidate_slots_cache.failed", out.LogFields{
				"healthcare_service_id": msgJson.ID,
				"error":                 err.Error(),
			})
		}

		if err := l.useCase.InvalidateHealthcareServiceCache(invalidateCtx, msgJson.ID); err != nil {
			l.logger.Error("healthcare_service.invalidate_healthcare_service_cache.failed", out.LogFields{
				"healthcare_service_id": msgJson.ID,
				"error":                 err.Error(),
			})
		}
